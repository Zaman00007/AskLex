...
interface HeaderProps {
  ...
  onNewCase: () => void;
  onLogout: () => void;
  onReport: () => void; // 👈 Added this
}

const Header: React.FC<HeaderProps> = ({ onMenuClick, onNewCase, onLogout, onReport }) => {
  return (
    <header className="fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-50">
      <div className="flex items-center justify-between px-4 py-3 lg:px-6">
        ...
        <div className="hidden lg:flex items-center space-x-4">
          ...
          <button
            onClick={onNewCase}
            className="flex items-center space-x-2 bg-legal-navy text-white px-4 py-2 rounded-lg hover:bg-blue-800 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
